...
// Ghost Shell Anty (Polish #1): C-bridge to GhostShellConfig for the
// HTTP/2 SETTINGS GREASE patch in SendInitialData(). Declared at file
// scope (C++ disallows extern "C" inside function bodies).
//
// Declarations only — the weak fallback DEFINITIONS live in
// boringssl/extensions.cc (the single owner). On Windows/lld-link,
// having two TUs each provide a weak definition causes a "duplicate
// symbol" link error rather than ELF-style folding. So we only
// declare here and let the linker resolve to Blink's strong def
// (chrome.exe) or BoringSSL's weak fallback (tool binaries).
extern "C" bool GhostShell_IsActive();
extern "C" int  GhostShell_GetRandomSeed();
...
void SpdySession::SendInitialData() {
  ...
  if (enable_http2_settings_grease_) {
    // Ghost Shell Anty (Polish #1): per-profile-stable GREASE.
    // Default Chromium uses base::RandGenerator for both id and value
    // — that means the HTTP/2 fingerprint hash (Akamai / Cloudflare
    // observe it) varies between every TCP connection of the same
    // profile, which itself is unusual. Real browsers reuse the
    // GREASE id for the lifetime of a process — different profiles
    // see different values, but within one profile they're stable.
    // We mirror that pattern by deriving the id+value from the
    // GhostShellConfig random seed via a SplitMix64 mix.
    //
    // GhostShell_GetRandomSeed/IsActive are declared at file scope
    // (top of this file) as weak-linkage extern "C" — strong defs
    // win in chrome.exe; weak fallback returns 0/false in tool
    // binaries that don't link Blink.
    spdy::SpdySettingsId greased_id;
    uint32_t greased_value;
    if (GhostShell_IsActive()) {
      uint64_t st = static_cast<uint64_t>(
          static_cast<uint32_t>(GhostShell_GetRandomSeed()));
      // SplitMix64 — same construction used elsewhere in Ghost Shell.
      st += 0x9E3779B97F4A7C15ULL;
      uint64_t z = st;
      z = (z ^ (z >> 30)) * 0xBF58476D1CE4E5B9ULL;
      z = (z ^ (z >> 27)) * 0x94D049BB133111EBULL;
      z =  z ^ (z >> 31);
      // Map to the GREASE id format defined by RFC9113 §5.5: settings
      // with id 0x0a0a + 0x1000*x + 0x0010*y where x,y in [0,15].
      greased_id = static_cast<spdy::SpdySettingsId>(
          0x0a0a + 0x1000 * ((z >> 4) & 0xf) + 0x0010 * (z & 0xf));
      greased_value = static_cast<uint32_t>(z >> 32);
    } else {
      greased_id = 0x0a0a +
