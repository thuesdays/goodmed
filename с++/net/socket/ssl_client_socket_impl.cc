...
// Ghost Shell Anty (Tier 1 #4): per-profile JA3 cipher list and
// supported_groups extension. Declared as extern "C" because
// GhostShellConfig lives in Blink (which net/ can't include) — the
// real implementations are exported from Blink's platform target via
// PLATFORM_EXPORT in ghost_shell_config.h. Weak fallback stubs in
// boringssl/extensions.cc keep tool binaries linkable.
extern "C" int GhostShell_GetTLSCipherSuites(unsigned short* out, int max);
extern "C" int GhostShell_GetTLSSupportedGroups(unsigned short* out, int max);
extern "C" bool GhostShell_IsActive();
...
int SSLClientSocketImpl::Init() {
  ...
  // === GHOST SHELL ANTY (Tier 1 #4) ===
  // Per-profile JA3 cipher list. The default selection above mirrors
  // upstream Chromium; here we override it with a profile-specific
  // ordering so two profiles produce distinct JA3 hashes for the same
  // server. We build the OpenSSL "name1:name2:..." command from the
  // wire-format uint16 IDs returned by GhostShell_GetTLSCipherSuites,
  // mapping each through SSL_get_cipher_by_value -> SSL_CIPHER_get_name.
  // Unknown / unsupported IDs are skipped silently so a stale profile
  // can't block the handshake. If the resulting list is empty (all IDs
  // unrecognised) we keep the default — better a working connection
  // with the default JA3 than a broken one.
  if (GhostShell_IsActive()) {
    constexpr int kMaxGsCiphers = 64;
    unsigned short gs_ciphers[kMaxGsCiphers];
    int gs_n = GhostShell_GetTLSCipherSuites(gs_ciphers, kMaxGsCiphers);
    if (gs_n > 0) {
      std::string gs_cmd;
      gs_cmd.reserve(gs_n * 32);
      for (int i = 0; i < gs_n; ++i) {
        const SSL_CIPHER* c = SSL_get_cipher_by_value(gs_ciphers[i]);
        if (!c) continue;
        const char* name = SSL_CIPHER_get_name(c);
        if (!name || !*name) continue;
        if (!gs_cmd.empty()) gs_cmd.push_back(':');
        gs_cmd.append(name);
      }
      if (!gs_cmd.empty()) {
        if (!SSL_set_strict_cipher_list(ssl_.get(), gs_cmd.c_str())) {
          // Profile-specific list failed to apply (probably one of the
          // names is wrong on this build). Fall through with the
          // upstream default already set above — don't fail the socket.
          LOG(WARNING) << "GhostShell: per-profile cipher list rejected, "
                          "falling back to defaults: " << gs_cmd;
        }
      }
    }

    // Per-profile supported_groups (TLS 1.3 named curves). BoringSSL's
    // SSL_set1_group_ids takes a uint16_t array directly. Empty list ->
    // no override. Mismatched / unknown IDs would be caught by BoringSSL;
    // we treat any failure as soft and log it.
    constexpr int kMaxGsGroups = 16;
    unsigned short gs_groups[kMaxGsGroups];
    int gs_g = GhostShell_GetTLSSupportedGroups(gs_groups, kMaxGsGroups);
    if (gs_g > 0) {
      // SSL_set1_group_ids expects const uint16_t* — same memory layout as
      // unsigned short on every supported platform, but cast explicitly so
      // a weird host with a wider unsigned short would surface at compile
      // time rather than corrupt the call.
      static_assert(sizeof(unsigned short) == sizeof(uint16_t),
                    "GhostShell TLS groups bridge expects 16-bit shorts");
      const uint16_t* gs_groups_u16 =
          reinterpret_cast<const uint16_t*>(gs_groups);
      if (!SSL_set1_group_ids(ssl_.get(), gs_groups_u16,
                              static_cast<size_t>(gs_g))) {
        LOG(WARNING) << "GhostShell: per-profile supported_groups rejected, "
                        "falling back to defaults";
      }
    }
  }
  // === END GHOST SHELL ANTY (Tier 1 #4) ===
  ...
  // Ghost Shell Anty: extension permutation is now seeded directly
  // inside BoringSSL via the C-bridge to GhostShellConfig — no
  // per-handshake plumbing here. See third_party/boringssl/src/ssl/
  // extensions.cc::ghost_shell_fill_seeds_from_config.
