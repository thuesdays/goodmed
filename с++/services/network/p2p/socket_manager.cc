...
// Ghost Shell Anty: C-bridge to GhostShellConfig living in
// third_party/blink/renderer/platform. The network process can't
// include Blink headers directly, so we just declare the two
// symbols Blink exports for non-Blink consumers. Linker resolves
// them at build time.
...
// Ghost Shell Anty: drop interfaces in HOST scope (private RFC1918
// / link-local / loopback) from the network list before the renderer
// enumerates ICE candidates. The result: renderer sees only publicly-
// routable addresses (in practice, with a proxy: none — so the only
// ICE candidates come from STUN/TURN). LAN IPs cannot leak even on a
// privileged exploit.
//
// Gated on GhostShell_IsActive() — the C-bridge to GhostShellConfig.
// When the user launches the browser without --ghost-shell-payload,
// IsActive() returns false and the filter is bypassed (stock behaviour).
bool GhostShellShouldFilterHostCandidates() {
  return GhostShell_IsActive();
}

bool GhostShellIsHostScopeAddress(const net::IPAddress& a) {
  if (a.IsLoopback() || a.IsLinkLocal() || a.IsZero()) {
    return true;
  }
  // IPv4 RFC1918: 10/8, 172.16/12, 192.168/16, 100.64/10 (CGNAT).
  if (a.IsIPv4()) {
...
net::NetworkInterfaceList P2PSocketManager::DoGetNetworkList() {
  ...
  if (GhostShellShouldFilterHostCandidates()) {
    list.erase(std::remove_if(list.begin(), list.end(),
                              [](const net::NetworkInterface& i) {
                                return GhostShellIsHostScopeAddress(i.address);
                              }),
               list.end());
  }
  return list;
