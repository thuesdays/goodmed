...
namespace {
webrtc::SocketAddress GhostShellFakeLanIP(const webrtc::SocketAddress& original) {
  if (!GhostShell_IsActive()) return original;

  uint32_t s = static_cast<uint32_t>(GhostShell_GetRandomSeed());
  uint8_t octet3 = static_cast<uint8_t>((s >> 8) & 0xFF);
  uint8_t octet4 = static_cast<uint8_t>(s & 0xFF);
  if (octet4 == 0)   octet4 = 1;
  if (octet4 == 255) octet4 = 254;

  char fake_ip[16];
  snprintf(fake_ip, sizeof(fake_ip), "192.168.%u.%u",
           static_cast<unsigned>(octet3), static_cast<unsigned>(octet4));

  return webrtc::SocketAddress(fake_ip, original.port());
}
}  // namespace
...
void PeerConnection::OnTransportControllerCandidatesGathered(
  ...
  for (Candidates::const_iterator citer = candidates.begin();
    ...
    Candidate c = *citer;
    if (c.type() == IceCandidateType::kHost) {
      c.set_address(GhostShellFakeLanIP(c.address()));
    }
    std::unique_ptr<IceCandidate> candidate(
        new IceCandidate(transport_name, sdp_mline_index, c));
