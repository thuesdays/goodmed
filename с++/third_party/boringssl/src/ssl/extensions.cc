...
// Ghost Shell Anty: per-profile deterministic ClientHello extension
// permutation. Reads the seed directly via the C-bridge to
// GhostShellConfig (third_party/blink/renderer/platform). When the
// browser was launched without --ghost-shell-payload, GhostShell_IsActive
// returns false and we fall back to upstream RAND_bytes — preserves
// stock behaviour for unittests / non-Chromium callers.
//
// Same profile, every TLS handshake → identical extension order →
// identical JA3 hash. Different profiles → uncorrelated permutations.
// Weak forward declarations of the C-bridge to GhostShellConfig
// (third_party/blink/renderer/platform/ghost_shell_config.{h,cc}).
//
// Chromium tools that link BoringSSL but NOT Blink (e.g. the
// make_top_domain_list_variables generator) need the symbols
// resolvable. Weak stubs below provide a "patch off" fallback —
// they always return false / 0, which makes the seed path below
// fall through to the upstream RAND_bytes branch.
//
// In chrome.exe the strong definitions exported from Blink platform
// (PLATFORM_EXPORT) win at link time, so the patch fires normally.
extern "C" __attribute__((weak)) bool GhostShell_IsActive();
extern "C" __attribute__((weak)) int  GhostShell_GetRandomSeed();

// Tier 1 #4 — JA3 cipher / supported_groups bridge.
// ssl_client_socket_impl.cc is the only consumer in chrome.exe; weak
// stubs below let Blink-less tool binaries (which link BoringSSL but
// not net/socket) keep linking — the strong Blink-side definitions
// in ghost_shell_config.cc win at link time when both are present.
extern "C" __attribute__((weak)) int GhostShell_GetTLSCipherSuites(
    unsigned short* out, int max);
extern "C" __attribute__((weak)) int GhostShell_GetTLSSupportedGroups(
    unsigned short* out, int max);

// Weak stubs — only used when no strong definition is provided
// (i.e. when this object is linked into a Blink-less tool binary).
extern "C" __attribute__((weak)) bool GhostShell_IsActive() {
  return false;
}
extern "C" __attribute__((weak)) int GhostShell_GetRandomSeed() {
...
static bool ghost_shell_fill_seeds_from_config(uint8_t *out, size_t len) {
  if (!GhostShell_IsActive()) {
    return false;
  }
  // Seed = the per-profile random_seed from GhostShellConfig.
  // Use it as input to SplitMix64 — fast, well-mixed, deterministic.
  // Each successive iteration yields the next 64 bits of keystream.
  uint64_t state = static_cast<uint64_t>(
      static_cast<uint32_t>(GhostShell_GetRandomSeed()));
  if (state == 0) return false;  // Empty seed → fall back to RAND_bytes.
  size_t i = 0;
  while (i < len) {
    ...
    z = z ^ (z >> 31);
    const size_t take = (len - i < 8) ? (len - i) : 8;
    std::memcpy(out + i, &z, take);
    i += take;
  }
  return true;
}
...
bool ssl_setup_extension_permutation(SSL_HANDSHAKE *hs) {
  ...
  // Ghost Shell: deterministic seed wins if set; otherwise fall back
  // to RAND_bytes for callers that don't go through Chromium's
  // CommandLine layer (BoringSSL unit tests, internal tools, etc.).
  bool seeds_filled = ghost_shell_fill_seeds_from_config(
      reinterpret_cast<uint8_t *>(seeds), sizeof(seeds));
  if (!seeds_filled &&
