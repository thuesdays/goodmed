...
AtomicString NavigatorLanguage::language() {
  if (GhostShellConfig::GetInstance().IsActive()) {
    // Вызываем наш же метод languages(), чтобы взять первый язык из подмененного списка
...
const Vector<String>& NavigatorLanguage::languages() {
  ...
  if (GhostShellConfig::GetInstance().IsActive()) {
    ...
    // Получаем массив языков из конфига
  ...
  if (!languages_dirty_) return languages_;
  languages_.clear();
  
  String accept_languages = GetAcceptLanguages();
  
  // ИСПРАВЛЕНИЕ: Новый API возвращает Vector<String> напрямую
  languages_ = accept_languages.Split(',');
  
  for (auto& lang : languages_)
    lang = String(lang.StripWhiteSpace().Replace('_', '-'));
