...
// Ghost Shell Anty: per-profile sub-pixel offset to canvas
// measureText().width. The Font allowlist is applied earlier in
// FontCache::GetFontData (Sprint 11), but the same per-profile width
// offset must also flow through TextMetrics so canvas-text fingerprint
// signatures (e.g. fingerprint.com's text-width vector) stabilise per
// profile and differ between profiles. The offset is small (sub-pixel)
// so it doesn't break layout-style measurement use cases.
...
void TextMetrics::Update(const Font* font,
  ...
  // === GHOST SHELL (Tier 1 #3) ===
  // Apply per-profile sub-pixel font width offset. The base offset
  // comes from GhostShellConfig::GetFontWidthOffset() and is multiplied
  // by a deterministic per-string hash so the same string measures the
  // same way across calls (canvas fingerprint stability) but different
  // strings differ. Skip empty text to keep measureText("") at 0.
  if (real_width > 0.0) {
    auto& gs_cfg = GhostShellConfig::GetInstance();
    if (gs_cfg.IsActive()) {
      const double base_off = gs_cfg.GetFontWidthOffset();
      if (base_off != 0.0) {
        // Per-string deterministic salt: SplitMix-style mix of
        // characters + profile seed. Output range ≈ [-1, +1] which
        // keeps the perturbation under one display pixel.
        uint64_t h = static_cast<uint64_t>(gs_cfg.GetRandomSeed());
        h ^= 0x9E3779B97F4A7C15ULL;
        for (unsigned i = 0; i < text_.length(); ++i) {
          h ^= static_cast<uint64_t>(text_[i]);
          h *= 0xBF58476D1CE4E5B9ULL;
          h = (h >> 27) | (h << 37);
        }
        const double mix = static_cast<double>(
            static_cast<int32_t>(h & 0xFFFFFFFFu)) / 2147483648.0;
        real_width += base_off * mix;
        if (real_width < 0.0) real_width = 0.0;
      }
    }
  }
  // === END GHOST SHELL ===
