...
Event::Event(const AtomicString& event_type,
  ...
  // === GHOST SHELL: Jitter Pointer/Mouse Event Timestamps ===
  if (GhostShellConfig::GetInstance().IsActive() && !platform_time_stamp_.is_null()) {
    if (event_type == event_type_names::kPointermove ||
        ...
        event_type == event_type_names::kMouseenter ||
        event_type == event_type_names::kMouseleave ||
        event_type == event_type_names::kClick ||
        event_type == event_type_names::kAuxclick ||
        event_type == event_type_names::kContextmenu ||
        event_type == event_type_names::kDblclick ||
        event_type == event_type_names::kWheel) {
      
      // Deterministic jitter based on the original timestamp so that
      // pointer and mouse compat events get the EXACT same jitter.
      int64_t micros = platform_time_stamp_.since_origin().InMicroseconds();
      // Pseudo-random hash
      int64_t hash = (micros * 1103515245L + 12345L);
      int64_t sign = (hash % 2 == 0) ? 1 : -1;
      int64_t jitter_us = sign * (100 + (hash % 301)); // ±100µs to ±400µs
      
      platform_time_stamp_ += base::Microseconds(jitter_us);
    }
  }
  // === END GHOST SHELL ===
}
