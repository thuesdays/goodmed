...
DOMRect* Element::GetBoundingClientRect() {
  ...
  // 1. Сохраняем результат в переменную вместо прямого return
  DOMRect* result = DOMRect::FromRectF(GetBoundingClientRectNoLifecycleUpdate());

  // --- GHOST SHELL CORE PATCH: GEOMETRY NOISE ---
  if (GhostShellConfig::GetInstance().IsActive()) {
    double offset = GhostShellConfig::GetInstance().GetRectOffset();
    // Добавляем микро-сдвиг (например, 0.005) чтобы уникализировать рендеринг шрифтов
    if (offset > 0.0) {
      result->setX(result->x() + offset);
      result->setY(result->y() + offset);
      result->setWidth(result->width() + offset);
      result->setHeight(result->height() + offset);
    }
  }
  // ----------------------------------------------
