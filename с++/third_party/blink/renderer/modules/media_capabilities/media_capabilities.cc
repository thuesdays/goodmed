...
ScriptPromise<MediaCapabilitiesDecodingInfo> MediaCapabilities::decodingInfo(
  ...
  // === GHOST SHELL MEDIA CODECS SPOOFING ===
  {
    auto& gs = blink::GhostShellConfig::GetInstance();
    if (gs.IsActive()) {
      String content_type;
      if (config->hasVideo() && config->video()->hasContentType()) {
        content_type = config->video()->contentType().ToAsciiLower();
      } else if (config->hasAudio() && config->audio()->hasContentType()) {
        content_type = config->audio()->contentType().ToAsciiLower();
      }

      String codec_key;
      if (content_type.contains("av01") || content_type.contains("av1"))
        codec_key = "av1";
      else if (content_type.contains("vp09") || content_type.contains("vp9"))
        codec_key = "vp9";
      else if (content_type.contains("avc1") || content_type.contains("h264"))
        codec_key = "h264";
      else if (content_type.contains("hvc1") || content_type.contains("h265") ||
               content_type.contains("hev1"))
        codec_key = "h265";

      if (!codec_key.empty()) {
        auto* result = MediaCapabilitiesDecodingInfo::Create();
        result->setSupported(gs.GetCodecSupported(codec_key));
        result->setSmooth(gs.GetCodecSmooth(codec_key));
        result->setPowerEfficient(gs.GetCodecPowerEfficient(codec_key));
