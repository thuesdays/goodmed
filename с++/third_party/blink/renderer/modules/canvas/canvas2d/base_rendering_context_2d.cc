...
ImageData* BaseRenderingContext2D::getImageDataInternal(
  ...
  // === GHOST SHELL CANVAS NOISE ===
  {
    ...
    if (gs.IsActive()) {
      const double noise_amp = gs.GetCanvasNoise();
      if (noise_amp > 0.0) {
        // LCG seeded per-call — varies jitter across repeated reads. Real
        // GPU readbacks aren't bit-identical either, so this MATCHES
        // real-hardware behavior, not just varies it.
        const int32_t seed = gs.GetRandomSeed();
        base::span<uint8_t> bytes_span = image_data->RawByteSpan();
        if (!bytes_span.empty()) {
          uint8_t* bytes = bytes_span.data();
          const size_t len = bytes_span.size();
          uint32_t rng = static_cast<uint32_t>(seed) ^ 0xA5A5A5A5u;
          // Iterate RGBA pixels — leave alpha (i+3) untouched so
          // transparency semantics stay identical.
          for (size_t i = 0; i + 3 < len; i += 4) {
            for (int c = 0; c < 3; ++c) {
              rng = rng * 1103515245u + 12345u;
              // Jitter range: +/- (noise_amp * 1.5 * 255), rounded
              double delta =
                  (static_cast<double>((rng >> 16) & 0xFFu) / 255.0) *
                      noise_amp * 255.0 * 3.0 -
                  noise_amp * 255.0 * 1.5;
              int32_t v = static_cast<int32_t>(bytes[i + c]) +
