...
bool BatteryManager::charging() {
  ...
  if (GhostShellConfig::GetInstance().IsActive() &&
      GhostShellConfig::GetInstance().HasBattery()) {
    return GhostShellConfig::GetInstance().GetBatteryCharging();
  }
  // === END GHOST SHELL ===
  return battery_status_.Charging(); // Capital C
}
...
double BatteryManager::chargingTime() {
  ...
  if (GhostShellConfig::GetInstance().IsActive() &&
      GhostShellConfig::GetInstance().HasBattery()) {
    double t = GhostShellConfig::GetInstance().GetBatteryChargingTime();
...
double BatteryManager::dischargingTime() {
  ...
  if (GhostShellConfig::GetInstance().IsActive() &&
      GhostShellConfig::GetInstance().HasBattery()) {
    double t = GhostShellConfig::GetInstance().GetBatteryDischargingTime();
    if (t < 0) return std::numeric_limits<double>::infinity();
    return t;
  }
  // === END GHOST SHELL ===
...
double BatteryManager::level() {
  ...
  if (GhostShellConfig::GetInstance().IsActive() &&
      GhostShellConfig::GetInstance().HasBattery()) {
    return GhostShellConfig::GetInstance().GetBatteryLevel();
  }
  // === END GHOST SHELL ===
  return battery_status_.Level(); // Capital L
}
