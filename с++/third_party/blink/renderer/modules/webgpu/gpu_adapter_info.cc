...
const String& GPUAdapterInfo::vendor() const {
  if (GhostShellConfig::GetInstance().IsActive()) {
    // NoDestructor создает объект один раз и безопасно "утекает" при закрытии
    static const base::NoDestructor<String> v(GhostShellConfig::GetInstance().GetGPUVendor());
    if (!v->empty()) return *v;
  }
  return vendor_;
}
...
const String& GPUAdapterInfo::architecture() const {
  if (GhostShellConfig::GetInstance().IsActive()) {
    static const base::NoDestructor<String> a(GhostShellConfig::GetInstance().GetGPUArch());
    if (!a->empty()) return *a;
  }
...
const String& GPUAdapterInfo::device() const {
  if (GhostShellConfig::GetInstance().IsActive()) {
    // Внимание: если GetGPUDevice() не объявлен в ghost_shell_config.h, 
    // закомментируй эти строки или добавь метод в заголовок.
    static const base::NoDestructor<String> d(GhostShellConfig::GetInstance().GetGPUDevice());
    if (!d->empty()) return *d;
  }
  return device_;
}
...
const String& GPUAdapterInfo::description() const {
  // description оставляем реальным — без него вычисляется через другие поля
  return description_;
}
