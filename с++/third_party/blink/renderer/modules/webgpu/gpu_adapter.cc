...
GPUAdapter::GPUAdapter(
  ...
  *propertiesChain = &powerProperties;
  ...
  // === GHOST SHELL WEBGPU SPOOFING ===
  auto& gs = blink::GhostShellConfig::GetInstance();
  if (gs.IsActive()) {
    String webgl_vendor = gs.GetUnmaskedVendor();
    String webgpu_vendor;
    wtf_size_t open  = webgl_vendor.rfind('(');
    wtf_size_t close = webgl_vendor.rfind(')');
    if (open != kNotFound && close != kNotFound && close > open) {
      webgpu_vendor = webgl_vendor.substr(open + 1, close - open - 1).ToAsciiLower();
    } else {
      webgpu_vendor = "unknown";
    }
    vendor_ = webgpu_vendor;
    device_ = gs.GetUnmaskedRenderer();
    architecture_ = "";
    description_ = "";
  }
