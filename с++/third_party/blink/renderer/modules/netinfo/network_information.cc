...
V8ConnectionType NetworkInformation::type() const {
  ...
  if (GhostShellConfig::GetInstance().IsActive()) {
    String t = GhostShellConfig::GetInstance().GetConnectionType();
    if (t == "bluetooth") return V8ConnectionType(V8ConnectionType::Enum::kBluetooth);
    if (t == "cellular") return V8ConnectionType(V8ConnectionType::Enum::kCellular);
    if (t == "ethernet") return V8ConnectionType(V8ConnectionType::Enum::kEthernet);
    if (t == "none") return V8ConnectionType(V8ConnectionType::Enum::kNone);
    if (t == "wifi") return V8ConnectionType(V8ConnectionType::Enum::kWifi);
    if (t == "wimax") return V8ConnectionType(V8ConnectionType::Enum::kWimax);
    if (t == "other") return V8ConnectionType(V8ConnectionType::Enum::kOther);
    if (t == "unknown") return V8ConnectionType(V8ConnectionType::Enum::kUnknown);
  }
  // === END GHOST SHELL ===
...
uint32_t NetworkInformation::rtt() {
  if (GhostShellConfig::GetInstance().IsActive()) {
    int rtt = GhostShellConfig::GetInstance().GetConnectionRtt();
    if (rtt >= 0) return static_cast<uint32_t>(rtt);
  }
  MaybeShowWebHoldbackConsoleMsg();
...
bool NetworkInformation::saveData() const {
  if (GhostShellConfig::GetInstance().IsActive()) {
    return GhostShellConfig::GetInstance().GetConnectionSaveData();
  }
  bool save_data =
