...
V8OrientationType ScreenOrientation::type() const {
  ...
  if (GhostShellConfig::GetInstance().IsActive()) {
    String t = GhostShellConfig::GetInstance().GetOrientationType();
...
uint16_t ScreenOrientation::angle() const {
  if (GhostShellConfig::GetInstance().IsActive())
    return static_cast<uint16_t>(GhostShellConfig::GetInstance().GetOrientationAngle());
  return angle_;
}
